...
| Reproducible build and regression checks | Complete | `REPRODUCIBILITY.md` records the current R/Bioconductor validation baseline and rolling-update procedure; `validate-book.yml` runs fast source checks, while `publish-book.yml` performs the full render and post-render link/navigation checks. |
