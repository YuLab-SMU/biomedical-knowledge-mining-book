...
GO analyses (`groupGO()`, `enrichGO()` and `gseGO()`) support organisms that have an `OrgDb` object available (see also [session 2.2](go-semantic-similarity.qmd#gosemsim-supported-organisms)).
...
Gene IDs can be mapped to gene Symbols by using the parameter `readable=TRUE` or [`setReadable()` function](identifier-utilities.qmd#sec-setReadable).


## GO Gene Set Enrichment Analysis

The `r Biocpkg("clusterProfiler")` package provides the `gseGO()` function for [gene set enrichment analysis](enrichment-foundations.qmd#gsea-algorithm) using gene ontology.
...
A complementary strategy is Bayesian term selection, which uses term-gene coverage to rank terms by posterior explanatory support instead of semantic similarity alone (see @sec-bayes-enrich).
...
```{r eval=FALSE}
#| label: simplify-enricher-example
...
- For online annotation refreshes, downloads, and release metadata, see [external-service troubleshooting](troubleshooting.qmd#external-services).
- For custom annotations or unsupported organisms, see [universal enrichment](universal-enrichment.qmd#universal-api).
- To reduce redundancy, continue to [GOSemSim](go-semantic-similarity.qmd) and [Bayesian term selection](bayesian-term-selection.qmd#sec-bayes-enrich).
