...
The `dotplot()` function accepts a parameter `by` for setting the scale of dot sizes. The default parameter `by` is setting to `geneRatio`, which corresponding to the `GeneRatio` column of the output. If it was setting to `count`, the comparison will be based on gene counts, while if setting to `rowPercentage`, the dot sizes will be normalized by `count/(sum of each row)`. Users can also map the dot size to other derived variable (see [Chapter 16](#clusterProfiler-dplyr)).
