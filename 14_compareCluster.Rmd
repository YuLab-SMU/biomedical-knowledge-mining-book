...
```{r comparecluster-genelists, fig.height=5, fig.width=10, fig.cap="(ref:compareClusterGeneListcap)", fig.scap="(ref:compareClusterGeneListscap)", echo=FALSE}
library(stringr)
p1 <- dotplot(ck) + scale_y_discrete(labels=function(x) str_wrap(x, width=50))
p2 <- dotplot(formula_res) + scale_y_discrete(labels=function(x) str_wrap(x, width=30))
cowplot::plot_grid(p1, p2, ncol=2, labels=c("A", "B"), rel_widths=c(.9, 1))
```
...
```{r comparecluster-formula, fig.height=4, fig.width=7,fig.cap="(ref:compareClusterFormulacap)", fig.scap="(ref:compareClusterFormulascap)"}
