...
are plotted. Users can change the parameter `showCategory` to
specify how many categories of each cluster to be plotted, and if
`showCategory` is set to `NULL`, the whole result will
be plotted. The `showCategory` parameter also allows passing a vector of selected categories to [plot pathways of interest](#showing-specific-pathways)
...
#| fig-cap: |
#|   **`cnetplot()` for comparing functional profiles of multiple gene clusters.** 
#|   Genes and functional categories (*i.e.*, pathways) are encoded as pies to distinguish different gene clusters.
cnetplot(ck)
```
