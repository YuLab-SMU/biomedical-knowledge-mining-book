# MeSH semantic similarity analysis {#sec-meshes-semantic-similarity}

| Aspect | MeSH semantic similarity |
|---|---|
| **Questions** | How similar are MeSH terms or gene annotations under the MeSH hierarchy and information-content model? |
| **Input** | MeSH IDs, gene IDs, MeSH annotation data, and the semantic data prepared for `meshes`. |
| **Methods** | Information-content and graph-structure measures including Resnik, Lin, Jiang, Schlicker, and Wang. |
| **Output** | Term or gene similarity scores and matrices for grouping or downstream evidence integration. |
| **Main limitations** | Results depend on MeSH hierarchy, annotation frequency, database release, and identifier coverage. Similarity captures shared annotation structure, not clinical equivalence or causality. |

Use this chapter after checking MeSH coverage and release information; for the shared method overview, see [semantic similarity](semantic-similarity.qmd#semantic-similarity-overview).
...
The `r Biocpkg("meshes")` package [@yu_meshes_2018] relies on `MeSHDb` to prepare semantic data for measuring similarity. `MeSHDb` can be downloaded from `r Biocpkg("AnnotationHub")` (see also `r Biocpkg("AHMeSHDbs")`) and about 200 species are available and are supported by the `r Biocpkg("meshes")` package. 
...
```{r echo=FALSE}
#| label: load-hsamd-cache
## saveRDS(hsamd, file="cache/hsamd.rds")
hsamd <- readRDS("cache/hsamd.rds")
```


## MeSH semantic similarity measurement {#meshes-semantic-similarity}
...
Lin [@lin_information-theoretic_1998] and Schlicker [@schlicker_new_2006]) and one graph-structure based method (i.e. Wang [@wang_new_2007]), to measure MeSH term semantic similarity. For algorithm details, please refer to [Chapter 1](semantic-similarity.qmd#semantic-similarity-overview).
...
- Apply the MeSH resource to [MeSH enrichment](mesh-enrichment.qmd#meshes-enrichment).
- Use the common [visualization map](enrichplot.qmd#sec-enrichplot) for returned result objects.
