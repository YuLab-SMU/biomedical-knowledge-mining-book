...
| Aspect | KEGG analysis |
|---|---|
| **Questions** | Which KEGG pathways or modules are associated with a selected gene set or coordinated across a ranked profile? |
| **Input** | Gene IDs in a KEGG-supported namespace and organism code; GSEA also requires a named ranked vector. |
| **Methods** | Pathway/module ORA and GSEA; optional visualization with `browseKEGG()` or `pathview()`. |
| **Main functions** | `enrichKEGG()`, `gseKEGG()`, `enrichMKEGG()`, `gseMKEGG()`, `search_kegg_organism()`, `bitr_kegg()`. |
| **Output** | `enrichResult` / `gseaResult` objects and optional pathway diagrams. |
| **Main limitations** | Organism code and `keyType` must match. Remote data availability and service policies can affect retrieval; record source/version and mapping coverage. |
...
KEGG is a remote service that can fail transiently, so the calls in this chapter are wrapped in the `retry()` helper defined in `_common.R`; readers who prefer a single attempt can simply drop the wrapper in their own analysis.

```{r eval=FALSE}
#| label: find-keyType
# Try 'kegg' as keyType with a dummy gene
retry(enrichKEGG(gene = "abc", organism = "dosa", keyType = "kegg"))
# Output:
...
# Try 'ncbi-geneid'
retry(enrichKEGG(gene = "abc", organism = "dosa", keyType = "ncbi-geneid"))
# Output:
# Error in KEGG_convert("kegg", keyType, species) :
#   ncbi-geneid is not supported for dosa ...

# Try 'uniprot'
retry(enrichKEGG(gene = "abc", organism = "dosa", keyType = "uniprot"))
# Output:
# Error in KEGG_convert("kegg", keyType, species) :
...
kk <- retry(enrichKEGG(gene         = gene_list_entrez,
                       organism     = "osa",
                       keyType      = "kegg", # or 'ncbi-geneid' if supported
                       pvalueCutoff = 0.05))

# For dosa (using RAP-IDs)
kk <- retry(enrichKEGG(gene         = gene_list_rap,
                       organism     = "dosa",
                       keyType      = "kegg",
                       pvalueCutoff = 0.05))
```
...
The `GSON` object or the file can be used in `enricher()` and `GSEA()` functions (via the `gson` parameter or by parsing it). For the knowledge-layer model, serialization, and multi-resource workflow with `gsonList`, see the dedicated [GSON chapter](gson.qmd#sec-gson).
...
kk <- retry(enrichKEGG(gene = gene, organism = kk_gson))
kk2 <- retry(gseKEGG(geneList = geneList, organism = kk_gson))
```
...
## KEGG ID Conversion {#sec-kegg-id-conversion}
...
```{r}
#| eval: false
kk2 <- retry(gseKEGG(geneList     = geneList,
                     organism     = 'hsa',
                     minGSSize    = 120,
                     pvalueCutoff = 0.05,
                     verbose      = FALSE))
...
## Topology-aware KEGG enrichment
...
```{r}
#| label: topology-demo-kegg
demo <- clusterprofiler_enrichit_demo()
```

```{r}
#| label: nseKEGG-example
kk_nse <- nseKEGG(
...
head(as.data.frame(kk_nse)[, c("ID", "Description", "NES", "p.adjust")], 3)
```

```{r}
#| label: mnseKEGG-example
kk_mnse <- mnseKEGG(
...
head(as.data.frame(kk_mnse)[, c("ID", "Description", "NES", "p.adjust")], 3)
```
...
#| eval: false
mkk <- retry(enrichMKEGG(gene = gene,
                         organism = 'hsa',
                         pvalueCutoff = 1,
                         qvalueCutoff = 1))
head(mkk)                   
...
## KEGG compound over-representation analysis {#clusterprofiler-kegg-compound-ora}

Pathways can also be analysed from the metabolite side. Passing the special value `organism = "cpd"` makes `enrichKEGG()` (and `gseKEGG()`) use the KEGG **compound**→pathway mapping instead of an organism's gene→pathway mapping, so the input is a vector of KEGG Compound IDs (`C` numbers) rather than gene IDs:

```{r}
#| label: kegg-compound-example
#| eval: false
cpd <- c("C00022", "C00024", "C00031", "C00033", "C00036")

res_cpd <- retry(enrichKEGG(gene = cpd,
                            organism = "cpd",
                            pvalueCutoff = 1,
                            qvalueCutoff = 1))
head(res_cpd)
...
The organism-related arguments do not apply here: `keyType` is taken from the compound gene-set object, and `organism` must be exactly `"cpd"` — it is a sentinel, not a species code. The `ID` column of the result holds `map`-prefixed pathway IDs and `Description` the pathway names.

::: {.callout-note}
KEGG retired the `/link/cpd/pathway` endpoint, which now answers `HTTP 400`; that had broken `organism = "cpd"` entirely. It was repaired in clusterProfiler 4.21.2 by using `/link/compound/pathway` instead, so update the package if this call fails to download. If it does fail, see @sec-failed-download in the FAQ chapter — a download failure and a mapping failure now report differently.
:::


## Visualize enriched KEGG pathways

The `r Biocpkg("enrichplot")` package implements [several methods](enrichplot.qmd#sec-enrichplot) to visualize enriched terms. Most of them are general methods that can be used on GO, KEGG, MSigDb, and other gene set annotations. Here, we introduce the `clusterProfiler::browseKEGG()` and `pathview::pathview()` functions to help users explore enriched KEGG pathways with genes of interest.
...
- For frozen/offline or combined resources, see [GSON](gson.qmd#sec-gson).
- For network failures, API changes, and provenance, see [external-service troubleshooting](troubleshooting.qmd#external-services).
- For plots, continue to [specialized views](enrichplot-specialized.qmd#sec-visualization-specialized) or the [visualization map](enrichplot.qmd#sec-enrichplot).
- For a different pathway resource, see [Reactome](reactome-enrichment.qmd#reactomepa).
