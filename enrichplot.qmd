...
Users can use 'fc_threshold' parameter to `cnetplot` function to filter genes to only include those with `|foldChange| > fc_threshold`. This allows user to plot only high fold-change genes (e.g., `fc_threshold = 1`).
