...
# Preparing and inspecting biological identifiers {#useful-utilities}

| Aspect | Identifier utilities |
|---|---|
| **Question** | Are the identifiers, namespaces, mappings, and annotation resources ready for a defensible enrichment test? |
| **Input** | Character gene/protein IDs, an `OrgDb` or external mapping source, and optional custom annotation tables. |
| **Functions** | `bitr()`, `bitr_kegg()`, `select()`, `read.gmt()`, and mapping/inspection helpers. |
| **Output** | Mapped and unmapped ID tables, explicit `TERM2GENE`/`TERM2NAME`, and inputs suitable for ORA, GSEA, or custom enrichment. |
| **Limitations** | Mapping is release- and species-dependent; one-to-many mappings can change counts, and unmapped IDs are not evidence of absent biology. Preserve the mapping summary. |

Use these utilities before choosing an enrichment engine; for common failures, see [troubleshooting](troubleshooting.qmd#faq).
...
Note: If you want to extract genes in your input gene list that are belong to a specific term/pathway, you can use the [geneInCategory](result-objects.qmd#clusterProfiler-dplyr) function.


### `bitr_kegg`: converting biological IDs using KEGG API {#sec-bitr_kegg}

The `bitr_kegg()` function supports ID conversion via the KEGG API, which is useful for species supported by KEGG but lacking an `OrgDb` object. For detailed usage and examples, please refer to the [KEGG ID Conversion](kegg-enrichment.qmd#sec-kegg-id-conversion) section in the KEGG analysis chapter.



## `setReadable`: translating gene IDs to human readable symbols {#sec-setReadable}

Some of the functions, especially those internally supported for [DO](disease-enrichment.qmd#dose-enrichment), [GO](go-enrichment.qmd#clusterprofiler-go), and [Reactome Pathway](reactome-enrichment.qmd#reactomepa), support a parameter, `readable`.  If `readable = TRUE`, all the gene IDs will be translated to gene symbols. The `readable` parameter is not available for enrichment analysis of KEGG or using user's own annotation. KEGG analysis using `enrichKEGG` and `gseKEGG`, which internally query annotation information from KEEGG database and thus support all species if it is available in the KEGG database. However, KEGG database doesn't provide gene ID to symbol mapping information. For analysis using user's own annotation data, we even don't know what species is in analyzed. Translating gene IDs to gene symbols is partly supported using the `setReadable` function if and only if there is an `OrgDb` available. The `setReadable()` function also works with `compareCluster()` output.
...
## Enrichr is an external service and the library name is not guaranteed to be
## stable, so retry the download and skip this example if it is unavailable
## rather than failing the whole book build.
gmt <- retry_download(url, destfile = "COVID19_GeneSets.gmt")

if (!is.null(gmt)) {
    covid19_gs <- read.gmt(gmt)
    head(covid19_gs)
}
```
...
There are many gene sets available online. After parsing by the `read.gmt()` function, the data can be directly used to perform enrichment analysis using `enricher()` and `GSEA()` functions (see also [Chapter 12](universal-enrichment.qmd#universal-api)).
