...
### Do not confuse a failed download with a failed mapping {#sec-failed-download}
...
* **unreachable network** — `Failed to reach the KEGG REST API at '<url>'. The network is unreachable (connection failed, timed out or DNS lookup failed). ...` Your IDs are fine; the machine could not reach KEGG (proxy, firewall, or a timeout on a large response).
* **HTTP error status** — `... the request returned HTTP status <code>. The KEGG REST API may have changed or is temporarily unavailable.` Either KEGG retired that endpoint (this is how a KEGG API change shows up) or the service is down; updating the package or using a local database is the fix.
* **empty response** — `... the response was empty. The requested data may not exist in KEGG ...`
* **empty annotation** — `No KEGG pathway annotation could be assembled for species '<x>': the downloaded gene/pathway table shares no ID with the pathway-name table.` The download succeeded but the two halves did not line up, which again usually means the KEGG response format changed.

Only if none of those appear should you suspect the identifiers themselves.
...
```{r eval=FALSE}
#| label: faq-local-kegg
library(clusterProfiler)
createKEGGdb::createKEGGdb("hsa")   # writes KEGG.db
x <- enrichKEGG(gene, organism = "hsa", use_internal_data = TRUE)
```
...
Note: Another solution is using the `filter` verb to extract a subset of the result as described in [Chapter 16](result-objects.qmd#clusterProfiler-dplyr).
...
## External services, downloads, and version boundaries {#external-services}
...
|---|---|---|---|
| KEGG REST / `enrichKEGG()` | Pathway and module mappings, ID conversion | Organism code, endpoint, access date, response status, package version, and mapping coverage | Build and reuse a local KEGG database with `createKEGGdb`, or use a saved GSON resource where appropriate. |
...
| `biomaRt`, AnnotationHub, and GOA downloads | Identifier or annotation acquisition | Host/release, query, access date, species, and package version | Cache the downloaded mapping or freeze it as `TERM2GENE`/`TERM2NAME` or GSON. |

For any remote call, distinguish these cases:

1. **Connection failure or timeout:** the machine did not obtain the resource; inspect proxy, firewall, DNS, or service availability.
2. **HTTP or API schema failure:** the endpoint or response format may have changed; retain the status and response, then check the package release or use a local resource.
3. **Empty or low-coverage result:** the request succeeded but the organism, identifier namespace, release, or biological scope may not match.

The reproducible record should include the exact downloaded or serialized resource when redistribution permits it. If the resource cannot be redistributed, retain its accession, release, access date, query, checksum where possible, and the mapping counts used in the analysis. The [reproducible report](reproducible-report.qmd#reproducible-report) provides the delivery checklist; the [KEGG chapter](kegg-enrichment.qmd#clusterprofiler-kegg) and [external-result import chapter](enrichplot-import.qmd#sec-visualization-import) show the corresponding APIs.

## Next steps

- Return to [the data contract](data-contract.qmd) for input and universe decisions.
- Use [Choose a route](choose-a-route.qmd) when the failure is methodological rather than technical.
- For package-specific navigation, see the [capability map](capability-map.qmd).
