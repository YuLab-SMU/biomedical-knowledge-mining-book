...
| Aspect | GO semantic similarity |
|---|---|
| **Questions** | How similar are GO terms, genes, or gene clusters under the GO graph and annotations for a selected species? |
| **Input** | GO IDs, gene IDs, gene clusters, an `OrgDb`, and `GOSemSimDATA` prepared with `godata()`. |
| **Methods** | Information-content, graph-based, and topology-based measures such as Resnik, Lin, Jiang, Wang, and TCSS. |
| **Output** | Similarity scores, matrices, and gene-cluster or term-cluster summaries. |
| **Main limitations** | Results depend on the GO release, species-specific annotation coverage, ontology branch, and the `godata()` settings. Similarity does not establish shared mechanism. |

Use this chapter after a GO result has been checked; for the shared method overview, see [semantic similarity](semantic-similarity.qmd#semantic-similarity-overview).
...
`r Biocpkg("GOSemSim")` implemented all methods described in [Chapter 1](semantic-similarity.qmd#semantic-similarity-overview), including four IC-based methods (Resnik, Lin, Rel, Jiang), one graph-based method (Wang), and the topology-based TCSS method [see @sec-tcss].
...
## TCSS: Topology-based Clustering Semantic Similarity {#sec-tcss}
...
5. **Compute ICA**: Within each cluster, the Information Content Adjusted score normalises each term by the cluster's maximum IC: $ICA(t) = IC(t) / \max\{IC(t') : t' \in cluster\}$. A cluster with a single member has no internal spread, which would make its ICA identically 1 no matter how general the term is; such clusters are instead normalised by the global maximum IC, so a very general term does not receive a maximally specific score.
...
Users can also use [`clusterProfiler::bitr`](identifier-utilities.qmd#bitr) to translate biological IDs.
...
- For GO enrichment objects, return to [GO enrichment](go-enrichment.qmd#clusterprofiler-go).
- For representative-term selection, compare [simplify and Bayesian selection](bayesian-term-selection.qmd#sec-bayes-enrich).
