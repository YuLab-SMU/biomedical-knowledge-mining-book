...
```{r Barplot, fig.height=4, fig.width=10, fig.cap="(ref:Barplotcap)", fig.scap="(ref:Barplotscap)", echo=FALSE}
p1 <- barplot(edo, showCategory=20) + theme_dose(18)
p2 <- mutate(edo, qscore = -log(p.adjust, base=10)) %>% 
    barplot(x="qscore") + theme_dose(18)

cowplot::plot_grid(p1, p2, ncol=2, labels=c("A", "B"), label_size=20)    
```
...
edo2 <- gseDO(geneList)
p1 <- dotplot(edo, showCategory=30) + ggtitle("dotplot for ORA") + theme_dose(18)
p2 <- dotplot(edo2, showCategory=30) + ggtitle("dotplot for GSEA") + theme_dose(18)
plot_grid(p1, p2, ncol=2, labels = LETTERS[1:2], label_size=20)
```
...
```{r cnetplotgeneral, fig.height=3, fig.width=7, fig.cap="(ref:cnetplotgeneralcap)", fig.scap="(ref:cnetplotgeneralscap)"}
set.seed(123)
...
cowplot::plot_grid(p1, p2, ncol=2, labels=LETTERS[1:2], label_size=20)    
```
...
(ref:treeplotscap) Heatmap plot of enriched terms.

(ref:treeplotcap) **Heatmap plot of enriched terms.** default (A), `foldChange=geneList` (B)
