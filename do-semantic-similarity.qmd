...
| Aspect | Disease Ontology semantic similarity |
|---|---|
| **Questions** | How similar are disease terms, gene products, or disease-associated gene clusters under Disease Ontology? |
| **Input** | DO IDs, gene IDs, gene clusters, and the DO annotation and ontology data used by DOSE. |
| **Methods** | Information-content and graph-based term, gene, and cluster similarity measures. |
| **Output** | Pairwise scores, similarity matrices, and cluster summaries that can support disease-term organization. |
| **Main limitations** | Results depend on Disease Ontology structure, annotation coverage, identifier mapping, and release. Similar disease annotations are not evidence of a shared causal mechanism. |

Use this chapter after checking the disease knowledge source and identifiers; for the shared method overview, see [semantic similarity](semantic-similarity.qmd#semantic-similarity-overview).
...
Accurate disease descriptions can discover new relationships between genes and disease, and new functions for previously uncharacterized genes and alleles.
...
Four methods for determining the semantic similarity of two terms based on the Information Content of their common ancestor term were proposed by Resnik [@philip_semantic_1999], Jiang [@jiang_semantic_1997], Lin [@lin_information-theoretic_1998] and Schlicker [@schlicker_new_2006]. Wang [@wang_new_2007] presented a method to measure the similarity based on the graph structure. Each of these methods has its own advantage and weakness. `r Biocpkg("DOSE")` supports all these methods to compute semantic similarity among DO terms and gene products. For algorithm details, please refer to [Chapter 1](semantic-similarity.qmd#semantic-similarity-overview).


In `r Biocpkg("DOSE")`, we implemented `doSim` for calculating semantic similarities between pairs of DO terms or between two sets of DO terms.
...
We also implement a plot function `simplot()` to visualize the similarity result as demonstrated in @fig-dose-simplot.




```{r}
#| label: fig-dose-simplot  
#| echo: false
#| fig-width: 5
#| fig-height: 4
#| message: false
...
#| fig-cap: |
#|   **Similarity matrix visualization**. 
#|   The `simplot()` function visualize similarity matrix as a heatmap.
simplot(s,
...
On the basis of semantic similarity between DO terms, `r Biocpkg("DOSE")` can also compute semantic similarity among gene products. `r Biocpkg("DOSE")` provides four methods called `max`, `avg`, `rcmax` and `BMA` to combine semantic similarity scores of multiple DO terms (as described in @sec-combine-methods). The similarities among genes and gene clusters which are annotated by multiple DO terms were also calculated by these combining methods. 
...
- Apply the similarity model to [disease enrichment](disease-enrichment.qmd#dose-enrichment).
- Then reduce or visualize result terms through [evidence integration](03-evidence-integration.qmd).
