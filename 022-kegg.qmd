...
The `GSON` object or the file can be used in `enricher()` and `GSEA()` functions (via `gson` parameter or by parsing it).
...
```{r}
ko2name <- function(ko) {
  url <- paste0("http://rest.kegg.jp/list/", ko)
  x <- readLines(url)
  y <- strsplit(x, "\t")
  data.frame(ko= sapply(y, "[", 1), name= sapply(y, "[", 2), stringsAsFactors=FALSE)
}
