...
# Semantic Similarity Analysis {#semantic-similarity-overview}

| Aspect | Semantic-similarity methods |
|---|---|
| **Questions** | How similar are two terms, gene products, or gene clusters under a particular ontology or annotation source? Which redundant enrichment terms should be grouped or reduced? |
| **Input** | Term IDs, gene IDs, gene clusters, ontology annotations, or an enrichment result with a supported similarity method. |
| **Methods** | Information-content, ontology-graph, annotation-overlap, and topology-based similarity; term clustering and simplification. |
| **Main functions** | `pairwise_termsim()`, `simplify()`, `mgoSim()`, `geneSim()`, and knowledge-specific similarity functions in GOSemSim, DOSE, and meshes. |
| **Output** | Similarity scores or matrices, term clusters, and reduced result objects. |
| **Main limitations** | Results depend on ontology structure, annotation coverage, species, and release. A similarity score measures shared annotation structure, not causal or functional equivalence. |

Use similarity to organize a result set after checking its input, background, and database provenance. For method-specific examples, continue to [GO](go-semantic-similarity.qmd#GOSemSim), [DO](do-semantic-similarity.qmd#DOSE-semantic-similarity), or [MeSH](mesh-semantic-similarity.qmd#meshes-semantic-similarity).
...
As GO allows multiple parents for each concept, two terms can share
parents through multiple paths. IC-based methods calculate the similarity of two GO terms based on the information content of their closest common ancestor term, which is also called the most informative common ancestor (MICA).
...
Graph-based methods use the topology of the GO graph structure to
...
Term $A$'s contribution to itself is defined as 1. After obtaining the S-values for all terms in $DAG_{A}$,
the semantic value of GO term A, $SV(A)$, is calculated as:
...
## Combine methods {#sec-combine-methods}
...
We developed several Bioconductor packages, including `r Biocpkg("GOSemSim")` [@yu2010; @yu_gosemsim_2020] for computing semantic similarity among GO terms, sets of GO terms, gene products and gene clusters (see also [Chapter 2](go-semantic-similarity.qmd#GOSemSim)), `r Biocpkg("DOSE")` [@yu_dose_2015] for Disease Ontology (DO) (see also [Chapter 3](do-semantic-similarity.qmd#DOSE-semantic-similarity)) and `r Biocpkg("meshes")` [@yu_meshes_2018] that based on Medical Subject Headings (MeSH) (see also [Chapter 4](mesh-semantic-similarity.qmd#meshes-semantic-similarity)).

## Next steps

- Apply the knowledge-specific implementations in [GOSemSim](go-semantic-similarity.qmd#GOSemSim), [DOSE](do-semantic-similarity.qmd#DOSE-semantic-similarity), or [MeSH](mesh-semantic-similarity.qmd#sec-meshes-semantic-similarity).
- Use term selection before [visualizing networks](enrichplot-networks.qmd#sec-visualization-networks).
