...
| Aspect | Universal/custom enrichment |
|---|---|
| **Questions** | How can enrichment be tested for an unsupported organism, ontology, pathway collection, or identifier namespace? |
| **Input** | A selected gene vector or named ranked vector plus `TERM2GENE` and optional `TERM2NAME`; specify an experiment-appropriate universe. |
| **Methods** | ORA via `enricher()` and GSEA via `GSEA()` using user-supplied gene sets from GMT, GOA, UniProt, CellMarker, or project annotations. |
| **Main functions** | `enricher()`, `GSEA()`, `read.gmt()`, `buildGOmap()`, `gson()`, `gsonList()`. |
| **Output** | Standard `enrichResult` / `gseaResult` objects for comparison, simplification, and plotting. |
| **Main limitations** | The user owns ID consistency, annotation completeness, term naming, universe choice, and provenance; these shape the tested gene sets and interpretation. |
...
Users can download gene-set collections in GMT format from [MSigDB](https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp) and use `read.gmt()` to parse the file for `enricher()` or `GSEA()`.
...
- For KEGG downloads, mapping coverage, and release metadata, see [external-service troubleshooting](troubleshooting.qmd#external-services).
- Package reusable annotations as [GSON](gson.qmd#sec-gson).
- For plots, use the [visualization map](enrichplot.qmd#sec-enrichplot).
- For analysis design, see [Choose a route](choose-a-route.qmd).
